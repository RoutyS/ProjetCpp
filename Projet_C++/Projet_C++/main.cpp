...
public:
	...
	{
		data.resize(height, std::vector<std::vector<unsigned char>>(width, std::vector<unsigned char>(channels, 0)));
...
//methode get
	...
	*/
	...
	Image operator-(const Image& other) const
	{
		if (width != other.width || height != other.height || channels != other.channels || model != other.model)
		{
			throw std::invalid_argument("les images ne sont pas compatibles pour une soustraction");
		}
	...
	Image operator^(const Image& other) const
	{
		if (width != other.width || height != other.height || channels != other.channels || model != other.model)
		{
			throw std::invalid_argument("les images ne sont pas compatibles pour la difference");
		}
